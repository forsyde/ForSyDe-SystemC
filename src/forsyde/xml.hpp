...
public:
    ...
    void traverse(sc_module* top) {
        // The top most node for types
        types_root = types_doc.allocate_node(node_element, const_data_type);
        types_doc.append_node(pn_node);

        this->_traverse(top);

        std::string types_path(path + "types.xml");
        std::ofstream outFile(types_path);
        if (!outFile.is_open())
            SC_REPORT_ERROR(types_path.c_str(), "file could not be opened to write the introspection output. Does the path exists?");

        outFile << "<?xml version=\"1.0\" ?>" << std::endl;
        outFile << "<!-- Automatically generated by ForSyDe -->" << std::endl;
        outFile << "<!DOCTYPE introspective_types >"  << std::endl;
        outFile << types_doc;
    }
...
private:
                  ...
                  const char* bound_process=NULL, const char* bound_port=NULL)
    {
        ...
        if (port != NULL)
        {
        	IntrospectiveType type(types_doc, types_root);
    ...
    void add_signal(introspective_channel* sig, xml_node<>* pn_node)
    {
        ...
        allocate_append_attribute(sig_node, const_type, sig->token_type(type));
...
private:
    ...
    //! The RapidXML DOM
    xml_document<> xml_doc, types_doc;
    xml_node<>* types_root;
