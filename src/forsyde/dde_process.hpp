...
template <typename T>
class DDE2DDE: public ForSyDe::signal<T,tt_event<T>>
{
public:
    DDE2DDE() : ForSyDe::signal<T,tt_event<T>>() {}
    DDE2DDE(sc_module_name name, unsigned size) : ForSyDe::signal<T,tt_event<T>>(name, size) {}
...
template <typename T>
class DDE_in: public ForSyDe::in_port<T,tt_event<T>,signal<T>>
{
public:
    DDE_in() : ForSyDe::in_port<T,tt_event<T>,signal<T>>(){}
    DDE_in(const char* name) : ForSyDe::in_port<T,tt_event<T>,signal<T>>(name){}
};
...
template <typename T>
class DDE_out: public ForSyDe::out_port<T,tt_event<T>,signal<T>>
{
public:
    DDE_out() : ForSyDe::out_port<T,tt_event<T>,signal<T>>(){}
    DDE_out(const char* name) : ForSyDe::out_port<T,tt_event<T>,signal<T>>(name){}
};
